<template>
  <div class="orderInformation">
    ...
    <section id="printSection" class="card">
      <b-button
        tag="router-link"
        type="is-text"
        outlined
      ...
      <b-button type="is-text" outlined size="is-small" @click="print">
      ...
      <div class="card-content">
        ...
        <table v-if="printing" class="table is-fullwidth">
          <thead>
            ...
            <b-table :data="products" hoverable mobile-cards>
              ...
              <template slot="footer">
                <div class="has-text-right tableFooter">
                  TOTAL ITEMS VALUE: {{ this.orderValue }} &euro;
                </div>
              </template>
            </b-table>
          </b-tab-item>
...
export default {
  ...
  methods: {
    ...
    print() {
      // Activates print section
      this.printing = true; // adds table from other tab which is not visible for printing
      ...
      .then((doc) => {
        if (doc.data()) {
          ...
          doc.data().products.forEach((item) => {
            this.orderValue = this.orderValue + item.price * item.quantity;
            this.products.push({
